...
import Quickcheck.ArbitraryFueled
...
To view the code for the derived generator, users can enable trace messages using the `quickcheck.deriving.arbitrary` trace class as follows:

```lean
set_option trace.quickcheck.deriving.arbitrary true
```
...
def mkBody (header : Header) (inductiveVal : InductiveVal) (generatorType : TSyntax `term) : TermElabM Term := do
  ...
  -- since some of Quickcheck's `Gen` combinators operate on generator functions
...
def mkAuxFunction (ctx : Deriving.Context) (i : Nat) : TermElabM Command := do
  ...
  -- (the `Quickcheck.Gen` type constructor applied to the name of the `inductive` type, plus any type parameters)
  let targetType ← mkInductiveApp ctx.typeInfos[i]! header.argNames
  let generatorType ← `($(mkIdent ``Quickcheck.Gen) $targetType)
